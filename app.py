...
def get_finallist(reviewlist,prod_detail):
    ...
    def negative_review(x):
        if x<=0:
            ...
            return 0
    data['high_positive_review']=data['Polarity'].apply(lambda x:high_polarity_analysis(x))
    data['positive_review']=data['Polarity'].apply(lambda x:positive_review(x))
    data['negative_review']=data['Polarity'].apply(lambda x:negative_review(x))
    df=pd.pivot_table(data,index=['Asin_Num'],values=['high_positive_review','positive_review','negative_review'],aggfunc=np.sum).reset_index()
    ...
    df['Total_Review_Scrapped']=df['positive_review']+df['negative_review']               
    df=df.merge(df1,how='outer',on='Asin_Num')
    df=df.rename(columns={'Rating':'Median_Rating'})  
    df=df.sort_values(by=['High_Positive_Review','Positive_review','Median_Rating'],ascending=False)
    ...
    col_name=['Rank','Product_Title','Price(INR)','Median_Rating','High_Positive_Review','Positive_review','Product_url']
